        ...
        try:
            while True:
                ...
                if line is None:
                    ...
                    #     self.clear_line = False
                    if self.clear_line:
                        self.delete("end-{}c".format(self.last_line_len), END)
                        self.clear_line = False

                    current_line = str(line)
                    self.insert(END, current_line)
                    self.last_line_len = len(current_line)
                    self.see("end-1c")
...
def show_img(img):
    ...
    canvas.delete("all")
    canvas.create_image((800 - w) // 2, (500 - h) // 2, image=root.preview, anchor=NW)
...
file_path = StringVar()
file_path.set("N/A")
Label(right_top_panel, text="Path of source images:").grid(columnspan=2, sticky="W")
Label(right_top_panel, textvariable=file_path, wraplength=150).grid(row=1, columnspan=2, sticky="W")
...
Entry(right_top_panel, width=5, textvariable=img_size).grid(row=2, column=1, sticky="W")
...
def load_images():
    ...
    fp = filedialog.askdirectory(initialdir=os.path.dirname(__file__), title="Select folder")
...
Button(right_top_panel, text=" Load images ", command=load_images).grid(row=3, column=0, columnspan=2, pady=(0, 5))

Separator(right_panel, orient="horizontal").grid(row=1, columnspan=2, sticky="we")
...
Label(right_sort_opt_panel, text="Sort methods:").grid(row=0, column=0, pady=5, sticky="W")
OptionMenu(right_sort_opt_panel, sort_method, "", *make_img.all_sort_methods).grid(row=0, column=1)

Label(right_sort_opt_panel, text="Aspect ratio:").grid(row=1, column=0, sticky="W")
...
rev_row = IntVar()
rev_sort = IntVar()
Checkbutton(right_sort_opt_panel, variable=rev_row, text="Reverse consecutive row").grid(row=2, columnspan=2,
                                                                                         sticky="W")
Checkbutton(right_sort_opt_panel, variable=rev_sort, text="Reverse sort direction").grid(row=3, columnspan=2,
                                                                                         sticky="W")
...
dest_img = None
Label(right_collage_opt_panel, text="Path of destination image").grid(row=0, columnspan=2, sticky="W")
Label(right_collage_opt_panel, textvariable=dest_img_path, wraplength=150).grid(row=1, columnspan=2, sticky="W")
...
Label(right_collage_opt_panel, text="Sigma: ").grid(row=3, column=0, sticky="W", pady=(10, 2))
Entry(right_collage_opt_panel, textvariable=sigma, width=8).grid(row=3, column=1, sticky="W", pady=(10, 2))
Label(right_collage_opt_panel, text="Color space: ").grid(row=4, column=0, sticky="W")
OptionMenu(right_collage_opt_panel, color_space, "",
           *make_img.all_color_spaces).grid(row=4, column=1, sticky="W")

Separator(right_collage_opt_panel, orient="horizontal").grid(row=6, columnspan=2, sticky="we", pady=(5, 5))
...
OptionMenu(collage_even_panel, ctype, "", *make_img.all_ctypes).grid(row=0, column=1, sticky="W")
Label(collage_even_panel, text="Duplicates: ").grid(row=1, column=0, sticky="W")
...
Separator(right_panel, orient="horizontal").grid(row=3, columnspan=2, sticky="we", pady=(0, 10))
...
Button(right_panel, text=" Save image ", command=save_img).grid(row=4, columnspan=2)
